...
@WebServlet("/orderConfirmation")   // Configure the request URL for this servlet (Tomcat 7/Servlet 3.0 upwards)
   ...
   //<!-- Start Main Top -->
    ...
    //<!-- End Main Top -->
          ...
          +"<div class=\"container\">"
                      +"<div class=\"row\">"
                          +"<div class=\"col-md-12\">"
                            +"<center><h1 class=\"m-b-20\"><strong><br>Dear  "+firstName[0]+" "+lastName[0]+",</strong><br><br></h1><center>"
                              +"<center><h1 class=\"m-b-20\"><strong><br>Thank you for shopping with us. Your order has been placed.</strong></h1><center>"
                              +"<h2><center>Your Purchase Order ID number is: #P "+orderID[0]+"</center></h2><br><br>"
                                  +"<p><a class=\"btn hvr-hover\" href=\"index.html\">Back To Shop</a></p></p>"
                          +"</div>"
                      +"</div>"
                  +"</div>"
              +"</li>"
  ...
  +"</div>"
    ...
    +"</body>");
      ...
      try (
         ...
         String[] postal = request.getParameterValues("postal");

         String[] cardtype = request.getParameterValues("card");
         String[] name = request.getParameterValues("name");
         String[] creditCardNo = request.getParameterValues("creditCardNo");
         String[] expiration = request.getParameterValues("expiration");
         String[] cvv = request.getParameterValues("CVV");
         //String[] orderID = request.getParameterValues("orderid");



           String sqlCustomerDetails = "INSERT INTO billing_address (firstName, lastName, email, country, address, postal, orderId) VALUES ("
                       + "'" + firstName[0] + "'" + ", " + "'" + lastName[0] + "'" + ", " + "'" + email[0] + "'" + ", " + "'" + country[0] + "'" + ", "+ "'" + address[0] + "'" + ", '" 
                       + postal[0] + "', "+orderID[0]+")";
            //out.println("<p>" + sqlCustomerDetails + "</p>");  // for debugging
            int countInserted = stmt.executeUpdate(sqlCustomerDetails);
            //out.println(countInserted + " records inserted.");

            String sqlInsertPay = "INSERT INTO payment (card, name, creditCardNo, expiration, cvv, orderId) VALUES ("
                     + "'" + cardtype[0] + "'" +  ", " + "'" + name[0] + "'" + ", " + "'" + creditCardNo[0] + "'" + ", " + "'" + expiration[0] + "'" + ", "+ "'" + cvv[0] + "'" + ", "+ orderID[0] + ")";
               //out.println("<p>" + sqlStr1 + "</p>");  // for debugging
               int countInsertedPay = stmt.executeUpdate(sqlInsertPay);
        


          String sqlUpdateOrderID = "update orderID set orderID = orderID + 1";
             //  out.println("<p>" + sqlStr2 + "</p>");  // for debugging
               int countUpdatedOrderID = stmt.executeUpdate(sqlUpdateOrderID);
          ...
          for (int i=0; i < cartItems.length; i++){
            ...
            String sqlUpdateShoeSize = "Update shoeSize set "+cartShoeSize[i]+" = "+cartShoeSize[i]+"- "+cartShoeQuantity[i]+" where shoeSize.SID = '"+cartItems[i]+"' ";
            //out.println("<p>Your SQL statement is: " + sqlUpdateShoeSize + "</p>"); // Echo for debugging
            int countUpdatedSize = stmt.executeUpdate(sqlUpdateShoeSize);
            //out.println(countUpdatedSize + " Shoe size updated. ");
